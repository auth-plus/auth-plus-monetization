...
class GetTotalCredit:
    ...
    def get_total_credit(self, external_id: UUID) -> float:
        console.info(f"Fetching total credit for external_id={external_id}")
        ...
        total = reduce(lambda a, b: float(a + b), amount_list, 0.0)
        console.info(f"Total credit for external_id={external_id} is {total}")
        return total
